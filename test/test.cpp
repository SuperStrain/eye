                                                  ...
                                                  ot_vpss_ext_chn_attr *pstVpssExtChnAttr)
{
    ...
    {
        LOGGER_ERROR(TEST, "VpssGrp%d is out of rang[0,%d].", VpssGrp, OT_VPSS_MAX_GRP_NUM);
        return TD_FAILURE;
    }

    if (VpssChn < 0 || VpssChn > OT_VPSS_MAX_CHN_NUM)
    {
        LOGGER_ERROR(TEST, "VpssChn%d is out of rang[0,%d]. ", VpssChn, OT_VPSS_MAX_CHN_NUM);
        return TD_FAILURE;
    }

    if (TD_NULL == pstVpssChnAttr && TD_NULL == pstVpssExtChnAttr)
    {
        LOGGER_ERROR(TEST, "null ptr,line%d.", __LINE__);
        return TD_FAILURE;
    }
    ...
    if (VpssChn < OT_VPSS_MAX_PHYS_CHN_NUM)
    {
        ...
        {
            LOGGER_ERROR(TEST, "ss_mpi_vpss_set_chn_attr failed with %#x", s32Ret);
            return TD_FAILURE;
        }
    }
    else
    {
        ...
        {
                LOGGER_NOTICE(TEST, "=========================VpssChn:%d", VpssChn);
                s32Ret = ss_mpi_vpss_set_ext_chn_attr(VpssGrp, VpssChn, pstVpssExtChnAttr);
                if (s32Ret != TD_SUCCESS)
                {
                    LOGGER_ERROR(TEST, "failed with %#x", s32Ret);
                    return TD_FAILURE;
                }
        }
    }
    ...
    if (VpssChn < OT_VPSS_MAX_PHYS_CHN_NUM)
    {
        ...
        if(s32Ret != TD_SUCCESS)
        {
            LOGGER_ERROR(TEST, "HI_MPI_VPSS_SetRotate failed with %#x!", s32Ret);
            return s32Ret;
        }
    }
    ...
    if (s32Ret != TD_SUCCESS)
    {
        LOGGER_ERROR(TEST, "ss_mpi_vpss_enable_chn failed with %#x", s32Ret);
        return TD_FAILURE;
    }
...
static void yuv_get_frame_process()
{
    ...
    while(1) {
        ...
        {
            LOGGER_ERROR(TEST, "ss_mpi_vpss_get_chn_frame chn(%d) fail,Error(%#x)", g_vpssChn, s32Ret);
            usleep(50*1000);
            continue;
        }

        td_u32 mmapSize = stFrmInfo.video_frame.stride[0] * stFrmInfo.video_frame.height * 3 / 2;
        LOGGER_NOTICE(TEST, "mmapSize=%u, stride0=%u, stride1=%u, height=%u, width=%u", 
        ...
        if(!vir_addr) {
            LOGGER_ERROR(TEST, "vir_addr is null");
            s32Ret = ss_mpi_vpss_release_chn_frame(g_vpssGrp, g_vpssChn, &stFrmInfo);
            if (TD_SUCCESS != s32Ret)
            {
                LOGGER_ERROR(TEST, "ss_mpi_vpss_release_chn_frame fail, chn(%d),Error(%#x)", g_vpssGrp, s32Ret);
            }
            usleep(1 * 1000 * 1000);
            continue;            
        }
        ...
        if (fp) {
            ...
            LOGGER_ERROR(TEST, "fopen %s failed!", filename.c_str());
        }
        i++;
...
static int yuv_frame_test()
{
    ...
    // if (TD_SUCCESS != s32Ret)
    // {
    //     LOGGER_ERROR(TEST, "Enable vpss chn %d failed!", g_vpssChn);
    //     return TD_FAILURE;
    // }
    LOGGER_NOTICE(TEST, "Enable vpss chn %d success!", g_vpssChn);
